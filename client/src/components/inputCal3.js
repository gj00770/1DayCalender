...
import { PlusMonth,MinusMonth,SelectGreyDate,ResetSelectDate,SetChecklist,SetSchedule,SetUpcoming,ShortCut,ShortCutPointer,CheckListStart} from '../redux/actions/action'
...
const getFirstDayofmonth =(year, month) =>{
 ...
 //select grey font
  const clickDate = (e)=>{
    ...
    props.CheckListStart(props.checkListYear, props.checkListMonth, e.target.innerHTML)
  }
  ...
  const selectFirstGrey =(e) =>{
    ...
    if(e.target.innerHTML !== '↑'){
        ...
        if(props.inputMonthEnd === 'January'){
            props.CheckListStart(props.checkListYear-1,'December',e.target.innerHTML)
        }
        else{
            props.CheckListStart(props.checkListYear, monthList[monthList.indexOf(props.checkListMonth)-1], e.target.innerHTML)
        }
    }
    else{
        
        if(props.inputMonthEnd === 'January'){
            props.CheckListStart(props.checkListYear-1, 'December', props.checkDateDisplay)
        }
        else{
            props.CheckListStart(props.checkListYear, monthList[monthList.indexOf(props.checkListMonth)-1], props.checkDateDisplay)
        }
    }
  ...
  const selectLastGrey =(e) =>{
    ...
    if(e.target.innerHTML !== '↓'){
        if(props.inputMonthEnd === 'December'){
            props.CheckListStart(props.checkListYear+1,'January',e.target.innerHTML)
        }
        else{
        props.CheckListStart(props.inputYearEnd,monthList[monthList.indexOf(props.checkListMonth)+1],e.target.innerHTML)
        }
    }
    else{
        if(props.inputMonthEnd === 'December'){
            props.CheckListStart(props.inputYearEnd+1,'January',props.checkDate)
        }
        else{
        props.CheckListStart(props.checkListYear,monthList[monthList.indexOf(props.checkListMonth)+1],props.checkDateDisplay)
        }
...
return(
    <div style={{backgroundColor:'white', display:'flex',height:'240px', border:'1px solid black'}}>
        <div>
        <Body> 
            <div>{getFirstDayofmonth(props.inputYearEnd,monthList.indexOf(props.inputMonthEnd)+1)}</div>
        <div style={{display:'flex'}}>
        <div>{props.inputYearEnd}년{monthList.indexOf(props.inputMonthEnd)+1}월{props.checkDate}일</div> <div onClick={selectFirstGrey}>&uarr;</div><div onClick={selectLastGrey}>&darr;</div>
        </div>
        <Weekend >
...
const mapDispatchToprops = (dispatch)=>{
  ...
  return {
      ...
      CheckListStart: (checkListYear,checkListMonth,checkDate)=> dispatch(CheckListStart(checkListYear,checkListMonth,checkDate)),
  }
} 

export default connect(mapStateToProps,mapDispatchToprops)(InputCal3)