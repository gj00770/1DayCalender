    ...
    <div style={{backgroundColor:'white', display:'flex',height:'240px', border:'1px solid black',textAlign:"center"}}>
        ...
        <div style={{overflow:'auto',paddingLeft:'20px',width:'60px'}}>
            {hourList.map((ele, index)=>(
                ele === props.calHourStartCal ||  ele === props.calHourEndCal ?
              <div key={index} style={{height:'50px',backgroundColor:'#4A78E6',display:'flex',alignItems:'center' , justifyContent:'center',color:'white'}} >{ele}</div>
              :
                
              props.calDateStartinput?
              <div key={index} style={{height:'50px',display:'flex',alignItems:'center' , justifyContent:'center'}} onClick={(e)=>props.clickHour(e)}>{ele}</div>
              :
              <div key={index} style={{height:'50px',display:'flex',alignItems:'center' , justifyContent:'center'}} onClick={(e)=>props.clickHourEnd(e)}>{ele}</div>
        ...
        <div style={{overflow:'auto',paddingLeft:'20px',width:'60px'}}>
            {minuteList.map((ele, index)=>(
                 ele === props.calMinuteEnd || ele === props.calMinuteStart?
              <div key={index} style={{height:'50px',backgroundColor:'#4A78E6',display:'flex',alignItems:'center' , justifyContent:'center',color:'white'}} >{ele}</div>
              :
              props.calDateStartinput?
              <div key={index} style={{height:'50px',display:'flex',alignItems:'center' , justifyContent:'center'}} onClick={(e)=>props.clickMinute(e)}>{ele}</div>
              :
              <div key={index} style={{height:'50px',display:'flex',alignItems:'center' , justifyContent:'center'}} onClick={(e)=>props.clickMinuteEnd(e)}>{ele}</div>
            ))}
        </div>

        <div style={{overflow:'auto',paddingLeft:'20px',width:'60px' }}>
            {props.noonEnd === 'noon' || props.noonStart === 'noon'? 
             
            <div>
             <div style={{height:'40px',backgroundColor:'#4A78E6',display:'flex', alignItems:'center' , justifyContent:'center',color:'white'}}>오전</div>
           {  props.calDateStartinput?
             <div style={{height:'40px',display:'flex', alignItems:'center' , justifyContent:'center'}} onClick={props.clickNoon}>오후</div>
             :
             <div style={{height:'40px',display:'flex', alignItems:'center' , justifyContent:'center'}} onClick={props.clickNoonEnd}>오후</div>
                }
             </div>
            :
            <div>
                {  props.calDateStartinput?
            <div style={{height:'40px',display:'flex', alignItems:'center' , justifyContent:'center'}} onClick={props.clickNoon}>오전</div>
            :
            <div style={{height:'40px',display:'flex', alignItems:'center' , justifyContent:'center'}} onClick={props.clickNoonEnd}>오전</div>
                }
             <div style={{height:'40px',backgroundColor:'#4A78E6',display:'flex',alignItems:'center' , justifyContent:'center',color:'white'}} >오후</div>
             </div>
...
export default connect(mapStateToProps,mapDispatchToprops)(EditCal)